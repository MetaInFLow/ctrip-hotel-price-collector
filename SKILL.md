...
- macOS 启动器记录本次浏览器的独立调试端口；Launch Services 已启动但 CDP、Playwright 或 Context 初始化失败时，会按 Profile 与端口清理对应 Chromium，避免留下停在 `about:blank` 的孤立窗口。
- 一个采集进程只使用一个浏览器上下文；任务会在该上下文中顺序处理所有酒店和日期，并在每个日期后保存结果。
