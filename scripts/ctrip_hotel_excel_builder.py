...
SUMMARY_HEADERS = ["酒店名称", "入住日期", "离店日期", "状态", "接口响应数", "房型行数", "详情页URL", "JSON文件", "错误"]
...
CURRENCY_HEADERS = {"价格", "原价"}
INTEGER_HEADERS = {"余房", "销售方案ID", "接口状态", "响应JSON字符数"}
...
def build(input_dir: Path, output_path: Path) -> None:
    ...
    _style_sheet(
        ...
        wrap_cols=["早餐及权益", "取消政策", "接口URL", "JSON文件", "详情页URL"],
    )
